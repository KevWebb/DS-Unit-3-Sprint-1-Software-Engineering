...
**- Making sure that the code is clear, and as efficient as possible with out sacrificing clarity. Also, making sure that the code is working as intended without error messages, and maybe added suggestions of different methods of producing the same results.

- Using containers could allow you to make sure all the computers are running the same way, so that switching computers will not cause your code to break, and it is easier to share files and notebooks between computers, making it easier to partner
### Part 8 - Turn it in!**
